...
describe('OwnerBusinessValidator', () => {
  it('validates name boundaries', () => {
    ...
    expect(() => OwnerBusinessValidator.validateName('A'.repeat(101))).toThrowError(
      InvalidOwnerNameError
    );
  ...
  it('validates email format when provided', () => {
    expect(() => OwnerBusinessValidator.validateEmail('not-an-email')).toThrowError(
      InvalidOwnerEmailError
    );
  ...
  it('validates phone format/length when provided', () => {
    ...
    expect(() => OwnerBusinessValidator.validatePhone('12345')).toThrowError(
      InvalidOwnerPhoneError
    );
